    ...
    <div className="d2-login-page">
      ...
      <aside className="d2-brand-panel">
        ...
        <div className="d2-brand-content">
          <div className="d2-brand-logo"><SuperMorpheusLogo size={56} /></div>
          <div className="d2-brand-text">
            <h2 className="d2-brand-headline">Join the<br /><span className="d2-brand-accent">Community</span></h2>
            <p className="d2-brand-tagline">Connect with like-minded professionals on a journey of personal growth and collective progress.</p>
          </div>
          <div className="d2-brand-values">
            {['Courage', 'Sincerity', 'Perseverance', 'Generosity', 'Progress', 'Truth'].map(v => (
              <span key={v} className="d2-value-tag">{v}</span>
            ))}
          </div>
        </div>
        <p className="d2-brand-footer">Super Morpheus &copy; {new Date().getFullYear()}</p>
      </aside>

      {/* Right — Signup Form */}
      <main className="d2-form-panel">
        <div className="d2-form-container d2-form-container--wide">
          <header className="d2-form-header">
            <h1 className="d2-form-title">Create your account</h1>
            <p className="d2-form-subtitle">Join the Super Morpheus community today</p>
          </header>
          ...
          <form className="d2-form" onSubmit={handleSubmit}>
            ...
            <button type="submit" className="d2-submit-btn" disabled={isSubmitting}>
              {isSubmitting ? <span className="d2-spinner" /> : 'Create Account'}
            </button>
          </form>

          <div className="d2-divider"><span className="d2-divider-line" /><span className="d2-divider-text">or</span><span className="d2-divider-line" /></div>

          <button className="d2-google-btn" type="button" onClick={() => navigate('/desktop-v2/onboarding')}>
          ...
          <p className="d2-signup-prompt">
            ...
            <Link to="/desktop-v2/login" className="d2-signup-link">Log in</Link>
          </p>
        </div>
      </main>
    </div>
